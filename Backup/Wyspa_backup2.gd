				...
				match side:
					...
					WEST:
						if id-1 >= 0 and id % w_size != 0:
							differencial = -1
				if differencial != null and picked_tile != null:
					if  evaluations == 0:
						ids_to_check.append(id+differencial)
					if all_cells[id+differencial]["collapsed"] == false:
						for i in all_cells[id+differencial]["domain"]:
							var reversed_walls = all_cells[id+differencial]["domain"][i][side-2].duplicate()
						...
						print(sides_to_remove)
					for i in sides_to_remove:
						all_cells[id+differencial]["domain"].erase(i)
